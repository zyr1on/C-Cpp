...
namespace algorithm {
    void drawCircleBresenham(SDL_Renderer* renderer, int centerX, int centerY, int radius) 
    {
        ...
        while (x >= y) {
            SDL_RenderDrawPoint(renderer, centerX + x, centerY + y); // 1. bölge
            SDL_RenderDrawPoint(renderer, centerX - x, centerY + y); // 2. bölge
            SDL_RenderDrawPoint(renderer, centerX + x, centerY - y); // 3. bölge
            SDL_RenderDrawPoint(renderer, centerX - x, centerY - y); // 4. bölge
            SDL_RenderDrawPoint(renderer, centerX + y, centerY + x); // 5. bölge
            SDL_RenderDrawPoint(renderer, centerX - y, centerY + x); // 6. bölge
            SDL_RenderDrawPoint(renderer, centerX + y, centerY - x); // 7. bölge
            SDL_RenderDrawPoint(renderer, centerX - y, centerY - x); // 8. bölge
            y++;
